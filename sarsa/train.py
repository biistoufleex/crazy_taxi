...
def plot_rewards_and_steps(rewards, steps):
    ...
    plt.ylabel('Value')
    plt.legend()
    plt.savefig('SARSA_rewards_and_steps.png')
    plt.close()
...
def plot_success_rate(success_rate):
    ...
    plt.ylim(0, 1)
    plt.savefig('SARSA_success_rate.png')
    plt.close()
...
def plot_steps_distribution(steps):
    ...
    plt.xlabel('Steps')
    plt.ylabel('Frequency')
    plt.savefig('SARSA_steps_distribution.png')
    plt.close()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train Taxi Driver Using the SARSA Algorithm")
    parser.add_argument(
        "--episodes",
        type=int,
        default=100000,
    ...
    execution_time, mean_reward = train(env, episodes, gamma, epsilon, max_epsilon, min_epsilon,
          epsilon_decay, alpha)
